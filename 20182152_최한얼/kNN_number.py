...
size, K = (308, 325),  15                                 # 숫자 영상 크기
nclass, nsample = 10, 20                                # 그룹수, 그룹당 샘플수

train_image = cv2.imread('images/train_20182152.jpg', cv2.IMREAD_GRAYSCALE)
if train_image is None: raise Exception("영상 파일 읽기 에러")
train_image = train_image[869:4174, 411:6640]

cells =[get_cell(train_image, j, i, size) for i in range(nclass) for j in range(nsample)]
nums = [find_number(c) for c in np.reshape(cells, (-1, 308, 325))]
